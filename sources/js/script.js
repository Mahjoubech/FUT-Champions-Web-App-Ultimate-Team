    ...
    positions.forEach( (item) => {
      ...
      item.addEventListener("click", () => {
        ...
        for (let i = 0; i < positions.length; i++) {
          if (
            positions[i].querySelector(".position span").textContent == attr
          ) {
...
// style des Tactics Formation les trois parties
  ...
  button.addEventListener("click", (e) => {
    ...
    if (!nameRegex.test(nameplayert.value)) {
      createToast(
        ...
        "fa-solid fa-triangle-exclamation",
        "Warning",
        "Invalid player name! Please enter only letters and spaces."
      );
    ...
    if (!positionRegex.test(posplayert.value)) {
      createToast(
        ...
        "fa-solid fa-triangle-exclamation",
        "Warning",
        "Invalid position! Please enter a number between 1 and 100."
      );
    ...
    playerStats.forEach((stat, index) => {
      ...
      if (!ratingRegex.test(stat.value)) {
        createToast(
          ...
          "fa-solid fa-triangle-exclamation",
          "Warning",
          `Invalid ${stat.name}! Please enter a number between 1 and 100.`
        );
        return;
      }
    });
    ...
    if (!file) {
      createToast(
        ...
        "fa-solid fa-triangle-exclamation",
        "Warning",
        "Please add a photo of the player."
      );
      return;
    }
    if (file) {
  ...
  dataplayers.forEach((data, index) => {
    if (data.position === "GK") {
      let content = `<div class = "prancipal">
             <div class="positions" id="${
               data.name
             }" onclick='ChangerPlayer(${JSON.stringify(data)})'>
            <div class="head-card">
              <div class="position">
                  <p>${data.rating}</p>
...
function ChangerPlayer(data) {
  ...
  if (selectedDiv) {
    ...
    if (data.position === "GK") {
      let content = ` <div class="positions" id="${
        data.name
      }" onclick='ChangerPlayer(${JSON.stringify(data)})'>
            <div class="head-card">
              <div class="position">
